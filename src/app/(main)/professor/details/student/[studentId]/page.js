...
import { getFeedbacksData } from "@/lib/api/services/tags";
...
export default function StudentDetails() {
  ...
  const aluno = useQuery({
    ...
    queryFn: () => getVisuPerfil(session.data.token, studentId),
  });

  const feedbacks = useQuery({
    queryKey: ["feedbacks_data"],
    queryFn: () => getFeedbacksData(session.data.token),
  });
  ...
  return (
    <div className="bg-base-200 flex justify-start flex-col items-center p-3 w-full h-screen md:max-w-[1000px] md:w-[1000px]">
      ...
      <div className="flex justify-center flex-col w-full items-start mb-6">
        ...
        <Link
          href={`/professor/details/student/${studentId}/history`}
